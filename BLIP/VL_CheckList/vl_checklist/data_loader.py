...
class DataLoader(object):
    ...
    def load_pos_and_neg_samples(self, corpus_names: list,type):
        ...
        for corpus_name in corpus_names:
            corpus[corpus_name] = []
            config = yaml.load(open(os.path.join(self.cur_dir, "VL_CheckList", 'corpus',  self.version, type, f'{corpus_name}.yaml'), 'r'), Loader=yaml.FullLoader)
            print(config["ANNO_PATH"])
            m = json.load(open(os.path.join(self.cur_dir, "VL_CheckList",config["ANNO_PATH"])))
            for x in m:
                path, texts_dict = x
                if corpus_name == "vg" or corpus_name == "vg_subj" or corpus_name == "vg_obj" or corpus_name == "vaw" :
                    path_1 = path.split("/")[0]
                    path = os.path.join(config["IMG_ROOT"], path_1, path)
                else:
                    path_1 = path.split("/")[0]
